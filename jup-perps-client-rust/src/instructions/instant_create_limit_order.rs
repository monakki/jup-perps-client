...
impl InstantCreateLimitOrder {
  pub fn instruction(&self, args: InstantCreateLimitOrderInstructionArgs) -> solana_program::instruction::Instruction {
    self.instruction_with_remaining_accounts(args, &[])
  }
  #[allow(clippy::arithmetic_side_effects)]
  #[allow(clippy::vec_init_then_push)]
  pub fn instruction_with_remaining_accounts(&self, args: InstantCreateLimitOrderInstructionArgs, remaining_accounts: &[solana_program::instruction::AccountMeta]) -> solana_program::instruction::Instruction {
    ...
    let accounts = InstantCreateLimitOrder {
          ...
          let args = InstantCreateLimitOrderInstructionArgs {
                                                              size_usd_delta: self.size_usd_delta.clone().expect("size_usd_delta is not set"),
                                                                  collateral_token_delta: self.collateral_token_delta.clone().expect("collateral_token_delta is not set"),
  ...
  pub fn invoke_signed(&self, signers_seeds: &[&[&[u8]]]) -> solana_program::entrypoint::ProgramResult {
          let args = InstantCreateLimitOrderInstructionArgs {
                                                              size_usd_delta: self.instruction.size_usd_delta.clone().expect("size_usd_delta is not set"),
                                                                  collateral_token_delta: self.instruction.collateral_token_delta.clone().expect("collateral_token_delta is not set"),
