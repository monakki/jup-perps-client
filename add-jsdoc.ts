...
const FUNCTION_DOCS = {
  ...
  identifyPerpetualsInstruction: 'Identifies Jupiter Perpetuals instruction type',
...
function addJSDocToFunction(content: string, functionName: string, description: string): string {
  // Patterns for finding functions
  const patterns = [
    // export async function fetchPool<...>
    new RegExp(`(export async function ${functionName}<[^>]*>\\([^)]*\\):[^{]*{)`, 'g'),
    // export async function fetchPool(
    new RegExp(`(export async function ${functionName}\\([^)]*\\):[^{]*{)`, 'g'),
...
function addJSDocToType(content: string, typeName: string, description: string): string {
...
async function addJSDocToFiles() {
